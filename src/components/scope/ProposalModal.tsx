...
export function ProposalModal({ proposal: initialProposal, scopeId, onClose }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: () => api.put(`/scope/${scopeId}/proposals/${proposal.id}`, {
  ...
  const acceptMutation = useMutation({
    mutationFn: () => api.post(`/scope/${scopeId}/proposals/${proposal.id}/accept`),
    onSuccess: (res) => {
      setProposal(res.data)
      setIsDirty(false)
  ...
  const rejectMutation = useMutation({
    mutationFn: () => api.post(`/scope/${scopeId}/proposals/${proposal.id}/reject`),
    onSuccess: (res) => {
      setProposal(res.data)
      setIsDirty(false)
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/scope/${scopeId}/proposals/${proposal.id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['scope-proposals', scopeId, proposal.issueKey] })
      onClose()
    },
  })
